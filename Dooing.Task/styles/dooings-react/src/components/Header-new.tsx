...
import type { HeaderProps, TaskCategory } from '../types';

const Header = ({ currentCategory, onCategoryChange, onNewTask, onToggleSidebar }: HeaderProps) => {
  ...
  const applyTheme = (theme: string) => {
    ...
    body.classList.remove(...themes.map(t => `theme-${t.value}`));
  ...
  const currentIndex = categories.findIndex(cat => cat.label === currentCategory);

  const handlePrevCategory = () => {
    const prevIndex = currentIndex > 0 ? currentIndex - 1 : categories.length - 1;
    onCategoryChange(categories[prevIndex].id as TaskCategory);
  };

  const handleNextCategory = () => {
    const nextIndex = currentIndex < categories.length - 1 ? currentIndex + 1 : 0;
    onCategoryChange(categories[nextIndex].id as TaskCategory);
  };
